;(function() {
  ...
  angular
    .module('kbitsApp.components.services')
    .factory('QueryService', [
      '$http', '$q', 'CONSTANTS', 'MessageService',QueryService
    ]);

  //////////////// factory
  function QueryService($http, $q, CONSTANTS, MessageService) {
    ...
    function query(method, url, params, data) {
      ...
      $http({
        ...
        if (data && !(data.config)) {
          MessageService.error('Server error occured.');
          deferred.reject(data);
        }else{
